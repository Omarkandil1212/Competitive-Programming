...
int main(){
    ...
    for(int i=n-1;i>=0;i--){
        add=0;
        if (nxt.lower_bound(seg[i].first.second+1)!=nxt.end()){
            v=nxt.lower_bound(seg[i].first.second+1)->second;
            add=sg.query(v,n-1).v;
        }
        dp[i]=seg[i].second+add;
        sg.update(i,dp[i]);
        if (nxt.find(seg[i].first.first)!=nxt.end()){
            nxt[seg[i].first.first]=i;
        }
        else{
            nxt.insert({seg[i].first.first,i});
        }
    }
    ll ans=0;
    for(int i=0;i<n;i++){
