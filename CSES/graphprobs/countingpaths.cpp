...
struct HeavyLightdecomp{ // head contains which path they belong to represented by a leader node
    ...
    ll query(int a, int b) {
        ...
        for (; head[a] != head[b]; b = parent[head[b]]) { // logn. while they are not on same path
            ...
            vals[pos[head[b]]]++;
            vals[pos[b]+1]--;
            sg.update(pos[head[b]],vals[pos[head[b]]]);
            sg.update(pos[b]+1,vals[pos[b]+1]);
            //cout<<pos[head[b]]<<" "<<vals[pos[head[b]]]<<" ";
            //cout<<pos[b]+1<<" "<<vals[pos[b]+1]<<"\n";
        ...
        vals[pos[a]]++;
        vals[pos[b]+1]--;
        //cout<<pos[a]<<" "<<vals[pos[a]]<<" ";
        //cout<<pos[b+1]<<" "<<vals[pos[b+1]]<<"\n";
        sg.update(pos[a],vals[pos[a]]);
        sg.update(pos[b]+1,vals[pos[b]+1]);
...
int main(){
    ...
    for(int i=0;i<n;i++){
        cout<<hld.sg.query(0,hld.pos[i]).v<<" ";
        //cout<<hld.pos[i]<<" "<<hld.vals[hld.pos[i]]<<'\n';
    }
    cout<<'\n';
